"""Service boundary for Weaveflow workflow operations.
...
from weaveflow.errors import MissingResultFileError, WorkspaceNotFoundError
from weaveflow.models import (
    Artifact,
    DoctorCheck,
    DoctorReport,
    Plan,
...
from weaveflow.paths import WorkspacePaths, ensure_workspace_dirs, workspace_paths
from weaveflow.store import TaskRepository, initialize_state
from weaveflow.yaml_io import read_yaml, write_yaml
...
def doctor_workspace(root: Path) -> DoctorReport:
    ...
    if not paths.root.exists():
        add(
            "error",
            "workspace_exists",
            f"workspace missing: {display_path(root, paths.root)}. Run `weaveflow init` first.",
            paths.root,
        )
...
def require_workspace(root: Path) -> WorkspacePaths:
    ...
    if not paths.exists():
        raise WorkspaceNotFoundError(
            "Weaveflow workspace not found. Run `weaveflow init` first."
        )
    return paths
