"""Typer CLI for ProjectOps Kernel."""
...
from projectops.json_io import CONTRACT_VERSION, dumps_json
from projectops.paths import workspace_paths


app = typer.Typer(help="Local-first ProjectOps Kernel MVP.")
...
def exit_with_error(error: ProjectOpsError) -> None:
...
@app.command()
def init() -> None:
    """Create a local .projectops workspace."""
    paths = workspace_paths(Path.cwd())
    already_exists = paths.exists()
    service.init_workspace(Path.cwd())
    if already_exists:
        typer.echo(
            f"ProjectOps workspace already exists at {paths.root}; "
            "ensured required files and directories."
        )
    else:
        typer.echo(f"Initialized ProjectOps workspace at {paths.root}")
...
def status(
    ...
    typer.echo(f".projectops exists: {exists}")
