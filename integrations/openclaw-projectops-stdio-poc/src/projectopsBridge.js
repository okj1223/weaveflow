...
export const CONTRACT_VERSION = "projectops.v1";
...
export async function initializeProjectOpsWorkspace(options) {
  ...
  if (result.code !== 0) {
    throw new Error(`ProjectOps workspace initialization failed: ${safeOneLine(result.stderr || result.stdout)}`);
  }
  return { ok: true };
}

export async function runProjectOpsStdioPoc(options) {
  const workspaceRoot = requireString(options?.workspaceRoot, "workspaceRoot");
  const pythonCommand = cleanOptionalString(options.pythonCommand) || "python3";
  const projectRoot = resolve(options.projectRoot || defaultProjectRoot());
  const timeoutMs = options.timeoutMs || DEFAULT_TIMEOUT_MS;
...
export function summarizeBridgeRun({ workspaceRoot, requests, processResult }) {
  ...
  const createdTaskExists = Boolean(
    taskId && existsSync(join(workspaceRoot, ".projectops", "tasks", taskId, "task_spec.yaml"))
  );
...
export function formatPocSummary(summary) {
  const lines = [
    `ProjectOps stdio POC: ${summary.ok ? "ok" : "failed"}`,
